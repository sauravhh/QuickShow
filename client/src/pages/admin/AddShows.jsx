...
const AddShows = () => {
    ...
    return nowPlayingMovies.length > 0 ? (
        <>
                                ...
                                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                                    {times.map((time) => (
                                        <div key={time} className='border border-primary px-2 py-1 flex items-center rounded'>
                                            <span>{time}</span>
                                            <DeleteIcon onClick={() => handleRemoveTime(date, time)} width={15} className='ml-2 text-red-500 hpver:text-red-700 cursor-pointer'/>
                                        </div>
                                    ))}
                                </div>
                            </li>
            ...
            <button className='bg-primary text-white px-8 py-2 mt-6 rounded hpver:bg-primary/90 transition-all cursor-pointer'>
                Add Show
            </button>
        </>
    ) : <Loading />
